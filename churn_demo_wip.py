...
with st.spinner('Please Wait...'):
    ...
    if file is not None:
        ...
        # #### Splitting Data
            ...
            ## plot created for internal validation of K means Clusters
            ...
            #     # PCA Scatterplot
            #     fig = px.scatter(clus_train, x=plot_columns[:, 0], y=plot_columns[:, 1], color=model3.labels_,  render_mode='svg')
            #     fig.update_layout(
            ...
            #plots for Outliers using isolation forest
            # # Isolation Forest
            ...
            random_forest_pipeline=estimate_pipeline(RandomForestClassifier(max_depth=20, min_samples_leaf=5, n_estimators=50, n_jobs=None, random_state=42), "Random Forest")
            ...
            #table created for Internal Validation of pipeline estimates
            ...
            st.subheader(f"Best Algorithm Model: {best_algorithm_model_name}")

            # Model Interpretations
            st.title('Model Interpretations')     
            ...
            def calc_Kfold(model):
                k = 10  # Define the number of folds for cross-validation

                st.subheader(f"K Fold Cross Validation\n")

                # Perform k-fold cross-validation for the training set
                cross_val_scores_train = cross_val_score(model, X_train, y_train, cv=k, scoring='accuracy')

                st.write("Train K Fold cross validation")
                fold_accuracies_train = []

                # Store fold accuracies for training in a list
                for fold, accuracy in enumerate(cross_val_scores_train, start=1):
                    fold_accuracies_train.append(f'Fold {fold}: Accuracy = {accuracy:.2f}')

                # Display fold accuracies in pairs for each row
                for i in range(0, len(fold_accuracies_train), 2):
                    st.write(f"{fold_accuracies_train[i]} {fold_accuracies_train[i + 1] if i + 1 < len(fold_accuracies_train) else ''}")

                # Calculate and display the mean accuracy for the training set
                mean_accuracy_train = np.mean(cross_val_scores_train)
                st.write(f'Train Mean Accuracy: {mean_accuracy_train:.2f}\n\n')

                # Perform k-fold cross-validation for the test set
                cross_val_scores_test = cross_val_score(model, X_test, y_test, cv=k, scoring='accuracy')

                st.write("Test K Fold cross validation")
                fold_accuracies_test = []

                # Store fold accuracies for testing in a list
                for fold, accuracy in enumerate(cross_val_scores_test, start=1):
                    fold_accuracies_test.append(f'Fold {fold}: Accuracy = {accuracy:.2f}')

                # Display fold accuracies in pairs for each row
                for i in range(0, len(fold_accuracies_test), 2):
                    st.write(f"{fold_accuracies_test[i]} {fold_accuracies_test[i + 1] if i + 1 < len(fold_accuracies_test) else ''}")

                # Calculate and display the mean accuracy for the test set
                mean_accuracy_test = np.mean(cross_val_scores_test)
                st.write(f'Test Mean Accuracy: {mean_accuracy_test:.2f}\n\n')

            random_forest_pipeline = calc_Kfold(RandomForestClassifier(max_depth=20, min_samples_leaf=5, n_estimators=50, n_jobs=None, random_state=42))
            ...
            train_roc_curve_fig.update_layout(
                ...
                legend=dict(x=0.75, y=0.02),  # Move the legend to the bottom right
